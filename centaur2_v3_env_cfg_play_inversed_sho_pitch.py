...
def swing_shoulders_event(env, env_ids, asset_cfg: SceneEntityCfg, amplitude_deg: float, freq_hz: float):
    """
    Fungsi custom untuk menggerakkan bahu secara sinusoidal.
    Bahu kiri dan kanan akan bergerak berlawanan arah (out of phase).
    """
    ...
    # Konversi amplitudo dari derajat ke radian
    amp_rad = math.radians(amplitude_deg)
    
    # Hitung posisi target dengan fungsi sinusoidal dasar
    target_pos = amp_rad * torch.sin(2 * math.pi * freq_hz * t)
    
    # Cari indeks spesifik untuk joint l_sho_pitch dan r_sho_pitch secara terpisah
    l_joint_indices, _ = asset.find_joints(["l_sho_pitch"])
    r_joint_indices, _ = asset.find_joints(["r_sho_pitch"])
    
    # Ambil target posisi persendian saat ini di memori
    current_targets = asset.data.joint_pos_target[env_ids].clone()
    
    # Eksekusi ke bahu kiri (sesuai target_pos)
    if len(l_joint_indices) > 0:
        current_targets[:, l_joint_indices] = target_pos.unsqueeze(1)
        
    # Eksekusi ke bahu kanan dengan nilai terbalik/negatif (-target_pos)
    if len(r_joint_indices) > 0:
        current_targets[:, r_joint_indices] = -target_pos.unsqueeze(1)
...
# ============================================================================
@configclass
class CentaurPlayEnvHandInversedCfg(CentaurBalancingEnvCfg):
